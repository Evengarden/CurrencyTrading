...
namespace CurrencyTrading.test.src.ServicesTests
{
    public class TradeServiceTests
    {
        ...
        public TradeServiceTests()
        {
            ...
            _tradeService = new TradeService(_tradeRepository.Object,_ownerRepository.Object,_lotRepository.Object, _mapper);
        }
        ...
        public async Task CreateTrade_ShouldReturnCustomErrorBalanceNotEnoughForBuyLot()
        {
            //arrange
            _lot.Type = Types.Buy;
            var balance = _owner.Balance.Where(b => b.Currency == "RUB").FirstOrDefault();
            balance.Amount = 0;
            _ownerRepository.Setup(u => u.GetUserAsync(_owner.Id)).ReturnsAsync(_owner);
            _ownerRepository.Setup(u => u.GetUserAsync(_buyer.Id)).ReturnsAsync(_buyer);
            _lotRepository.Setup(l => l.GetLotAsync(_lot.Id)).ReturnsAsync(_lot);
